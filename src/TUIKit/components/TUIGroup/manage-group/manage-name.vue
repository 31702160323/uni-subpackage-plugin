<template>
    <div class="name">
            ...
            <main>
                <header v-if="!isPC" class="edit-h5-header">
                    <aside class="left">
                        ...
                        <span>{{
                            TUITranslateService.t(`TUIGroup.修改群聊名称后，将在群内通知其他成员`)
                        }}</span>
                    </aside>
                    <span class="close" @click="close">{{ TUITranslateService.t(`关闭`) }}</span>
                </header>
                <div class="input-box">
                    <input
                        v-if="isEdit"
                    ...
                    />
                    <span v-if="!isPC">{{
                        TUITranslateService.t(`TUIGroup.仅限中文、字母、数字和下划线，2-20个字`)
                    }}</span>
                </div>
        ...
        <p v-if="!isEdit || !isPC" @click="close">
            ...
            <Icon v-if="isAuthor" :file="editIcon" width="14px" height="14px" />
        </p>
    </div>
</template>
